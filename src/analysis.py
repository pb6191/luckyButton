...
# hack to change textual numbers to numeric
...
# t-test to check differences between group means
ttest_rel(df["freq"], df["freq2"])
ttest_rel(df["freq2"], df["hover"])

ttest_ind(
    df[(df.condition == "deterministic")]["hover"],
    df[(df.condition == "probabilistic")]["hover"],
)
...
# create the plot for different conditions
